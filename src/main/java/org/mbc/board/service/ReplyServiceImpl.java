...
@Service
@RequiredArgsConstructor // final이 붙은 필드를 이용해 생성자를 만듦
@Log4j2
public class ReplyServiceImpl implements ReplyService {
    ...
    public PageResponseDTO<ReplyDTO> getListOfBoard(Long bno, PageRequestDTO pageRequestDTO) {
        ...
        Pageable pageable = PageRequest.of(
                ...
                // 페이지 번호가 0부터 시작하는데 프론트는 1부터 시작한다
                pageRequestDTO.getSize(),
                Sort.by("rno").ascending() // 댓글은 처음 등록한 것이 위로 올라오게 오름차순
        );
    ...
    public void modify(ReplyDTO replyDTO) {
        ...
        // 댓글 번호를 찾아서 엔티티 객체로 가져온다
        
        Reply reply = replyOptional.orElseThrow();
        
        reply.changeText(replyDTO.getReplyText()); // 댓글에 내용만 가져오게 함
        // 내용 수정용 메서드로 기록

        replyRepository.save(reply); // 있으면 update
        
        // 차후에 프론트에서 새로고침이 이루어질 예정
