...
public class BoardServiceImpl implements BoardService {
    ...
    public Long register(BoardDTO boardDTO) { // 조원이 실행코드를 만든다
        // 폼에서 넘어온 DTO가 DB에 기록되어야 함

        Board board = modelMapper.map(boardDTO, Board.class); // Entity가 dto로 변환
        Long bno = boardRepository.save(board).getBno();
        //                 .save =  insert into board~~~ => bno를 받는다
        
    return bno; // 프론트에 게시물 저장 후 번호가 전달 됨
    } // register종료
    ...
    public BoardDTO readOne(Long bno){
        ...
        // Optional null이ㅣ 나와도 예외처리 하지 않음
    ...
    public void modify(BoardDTO boardDTO) {
        ...
        Board board = result.orElseThrow(); // 성공시 (Null이 아닐때) 결과를 Entity로 저장
        board.change(boardDTO.getTitle(),boardDTO.getContent()); // 제목과 내용을 수정
        boardRepository.save(board); // DB에 pk가 있으면 update, 없으면 insert
