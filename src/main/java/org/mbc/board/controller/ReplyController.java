...
@RequiredArgsConstructor // p.555 추가
public class ReplyController {
    // 댓글용 컨틀롤러로 Rest방식으로 처리
    // 일반적으로 Contoller와 혼용해서 사용 됨

    private final ReplyService replyService; // p.555 추가
...
//    }
    ...
    public Map<String,Long> register(
       ...
       /* resultMap.put("rno",111L); // 리턴 테스트용 코드*/
        ...
        return resultMap; // 댓글 번호가 프론트로 넘어감
    ...
    // R -> 전체
    @GetMapping(value = "/list/{bno}") // http://localhost:80/replies/list/게시물번호
    ...
    public ReplyDTO getReplyDTO(@PathVariable("rno")Long rno){
        ...
        ReplyDTO replyDTO = replyService.read(rno); //select * from reply where rno = rno
    ...
    public Map<String,Long> remove(@PathVariable("rno")Long rno, @RequestBody ReplyDTO replyDTO){
        ...
        replyDTO.setRno(rno); // 댓글번호를 받아서 URL로 받은 값과 일치시킴
        replyService.modify(replyDTO); // 서비스 계층에서 update가 이루어짐
        Map<String,Long> resultMap = new HashMap<>(); // 결과 전송용 객체 생성
    ...
    // D
    @DeleteMapping("/{rno}") // http://localhost:80/replies/댓글번호
