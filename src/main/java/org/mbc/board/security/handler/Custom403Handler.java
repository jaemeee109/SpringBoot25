...
public class Custom403Handler implements AccessDeniedHandler {
    ...
    // 현재 사용자의 권한이 없거나
    // 특정 조건이 맞지않아 예외가 발생하는 403 처리용 클래스
    // form 태그를 통해서 전송된 결과를 처리하거나
    // Axios를 이용해서 비동기 처리 Ajax을 이용하므로 두가지 경우 다른 메세지 처리함
    // Ajax 비동기 처리 JSON 처리

    // <form> 태그의 요청이 403인 경우 로그인페이지로 이동할때 ACCESS_DENIED
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response,
        ...
        log.info("isJson:" + jsonRequest); // json 일때 처리 끝

        //일반요청시
        if(!jsonRequest) {
            response.sendRedirect("/member/login?error=ACCESS_DENIED");
        } // 403 예외 발생시 로그인 화면으로 이동하고 error 처리
